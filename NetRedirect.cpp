...
// 線程安全的deque，用於存儲封包
std::deque<std::shared_ptr<Packet>> packet_queue;
...
BOOL GetPackageBuffer() {
    while (true)
    {
        ...
        queue_cond.wait(lock, [] { return !packet_queue.empty(); });  // 等待有新的封包
        ...
        PacketAnalyze2(packet->data.get(), packet->len);
        PacketAnalyze(packet->data.get(), packet->len);
    }
...
void* __cdecl Mine_Memcpy(void* dest, const char* src, size_t count) {
    if (dest == PACKET_RECV_ADDRESS) {
        if (count > 0 && *AidAddress && *MAP_NAME != LOGIN_RSW) {
            ...
            packet->data.reset(new char[count], std::default_delete<char[]>());
            packet->len = count;
            memcpy(packet->data.get(), src, count);
